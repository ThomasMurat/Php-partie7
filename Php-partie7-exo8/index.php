    ...
    <body>
        <?php //on commence par vérifié si nos paramètres existent et si leur valeure sont correctes. Si c'est le cas on affichera les données transmisent.
            ...
            //On fait la même chose pour le cas ou on utilise la methode post
                ...
                if (isset($_FILES['myFile']) && $_FILES['myFile']['error'] == 0 && preg_match('/pdf/', $_FILES['myFile']['type'])) { //on vérifie qu'un fichier à bien été envoyé et si oui on l'affiche
                    ?><p><?= 'Mon fichier est : ' . $_FILES['myFile']['name'] ?></p>
                    <p><?= 'le type est : ' . $_FILES['myFile']['type'] ?></p><?php
                }
